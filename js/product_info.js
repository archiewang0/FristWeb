...
const countProdHander = function(){
	// const conutProd = Array.from(document.querySelector('div.cart_top_left > p').innerHTML)[1]
	...
	if (prods.length >= 2) {
        for (let idx = 1; idx < prods.length; idx++) {
			...
			itemCount = parseInt([...prods[idx].children][1].querySelector('input.quan_num').value)
			countProd += itemCount
        }
    }
...
const colorInputChecked = document.querySelectorAll('div.product_color div input')
...
const product_cartBtnSub = document.querySelector('div.pd_quan_outline>input.btn_sub')

const product_cartBtnAdd = document.querySelector('div.pd_quan_outline>input.btn_add')

//中間的input
const product_quanNumber = document.querySelector('div.pd_quan_outline>input.quan_num')
...
addToCart.addEventListener('click', (e) => {
    ...
    const imgUrl = e.target.parentNode.parentNode.parentNode.parentNode.querySelector('main.product_img>img').src
    ...
    const prodInfo = {
        prodName: formBody.querySelector('section.page_title>h1.button-title').innerHTML,
        ...
        quan: formBody.querySelector('div.product_quantity div.pd_quan_outline input.quan_num').value,
        subtotal: formBody.querySelector('div.product_subtotal div h6.subtotal_show').innerHTML.split(' ').shift(),
        imgUrl: imgUrl,
    }
    ...
    newAddProdBtnAdd.onclick = () => {
        ...
        //顯示在上面
        prodEl.querySelector('div.item_price').innerHTML = `${prodListSubtotal} NT$`
    ...
    newItemDelBtn.onclick = (e) => {
        ...
        if ([...cartList.children].length == 1) {
            ...
            document.querySelector('div.row_subtotal > h4:last-child').innerHTML = `0 NT$`
            // console.log("work1");
...
const prodImgChangeIcons = [...document.querySelectorAll('div.prodImgChangeIcon>button')]
...
function changeImage(e) {
    ...
    if (e.target == [...e.target.parentNode.children][1]) {
        ...
        const imgCount = [...document.querySelector('main.product_container>div.product_left').children].length

        if (prodImgCurrentindex > imgCount - 1) {
            prodImgCurrentindex = 0
        }

        const imgWidth = document.querySelector('main.product_container>div.product_left>div').getBoundingClientRect()
            .width
...
const changeNumber = function (e) {
    ...
    const imgNumber = e.target.parentNode.parentNode.parentNode.querySelector('div.img_number>h1')
...
prodImgChangeIcons.forEach((button) => button.addEventListener('click', changeImage))
